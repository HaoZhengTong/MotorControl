...
#define motor_brake motor_l=1;motor_r=1				  //双高电位
#define motor_software_stop motor_l=0;motor_r=0;CCAP0H=0  //电机软刹车 （断电）
#define motor_nopower motor_l=0;motor_r=0;CCAP0H=0xff	  //0xff
#define led_g CCAP1H									  //ledg灯	PWM1
#define led_y CCAP2H									  //ledy灯  PWM2
...
unsigned int code temp_data[91]=									   //温度对应值
...
,1000,963,927,893,861,830,800,771,743,717          //25
 ...
 ,351,340,330,319,309,300,291,282,273,265,257};    //55
                                                   //65
//保护信息
sbit motor_i = P1^2; //电机电流反馈 AD2		  //AD0	 电机工作电压值判定
sbit tem = P1^3; //热敏电阻 AD3				  //AD1	 温度保护值判定
sbit power_v = P1^4; //总电源电压 AD4		  //AD2	 总电源12v判定
//指示灯
sbit led_gg = P1^0; //黄灯 PWM1
sbit led_yy = P3^7; //绿灯 PWM2
//电机
sbit motor_l = P5^4; //电机正转 高有效	 1为高电，0为低电
sbit motor_r = P5^5; //电机反转 低有效	 1为高电，0为低电
sbit motor_pwm = P1^1; //电机速度 PWM0
sbit motor_a = P3^6; //电机编码器A		 测速入口
sbit motor_b = P3^2; //电机编码器B		 测速入口
//通讯
sbit io_t = P1^5; //发送标记位			   两个串口
...
//电机角度
long angle=0;//电机实时角度 
long angle_real;//记录真实角度（单位度），主要用于连续角度锁停模式下减少误差
...
long angle_target;//电机角度目标量，仅作中间值（中间值意思可能是说这个值不能认为设定，是程序运转产生的中间目标值）
long timer;//计时
...
unsigned char content[8];	//接受的指令存储？
...
/*led呼吸灯程序*/
			...
			{ 
				...
				{	
					...
					{ 
						motor_sangle_record=0;		  //使用完角度置0，不知道怎么恢复？只能用一次？
...
//获取速度程序			
	    ...
	    if(speed<0)
				speed_get();
/*AD转换程序*/
			ADC_CONTR=0x88+main_ad_n;		   //ADC_CONTR配置为1000 1000+2~4（供电，540个时钟周期转换一次，开始转换，选择P1.2~P1.4作为AD输入)
			main_ad_n++;
...
{ int temp_real_nn=0;//序号,也是温度
	...
	while(1)
	{
		if(temp_data[temp_real_nn]<temp_real_n)			 //数据表中第nn项小于测量值则返回温度
		{
			if(temp_real_nn-26>temp_safety_temp)		 //超出了第91项（65度），返回安全温度
				return temp_safety_temp;
			else										 //返回温度
				return temp_real_nn-26;
		}
		else
			temp_real_nn++;								  //提高温度（降低信号参数值）继续与实际温度比较
	}
}
...
void speed_get()
{
			angle_record[angle_record_n]=angle;										                             //将当前记录的角度存储到温度记录数组中
			if(angle_record_n==10)																				 //存满一轮后最后一位与新的第一位作差求速度
			{
			...
			angle_record_n++;																					 //指针加一，往后一位存下一个温度
...
//		UART_SendByte(RI);
//		UART_SendByte(io_r);
//		UART_SendByte(content_num);
	  if(io_r && content_num<8)//接收选位拉低
		{ 
			content[content_num]=SBUF;
			UART_SendByte(content[content_num]);
			UART_SendByte(content_num);
			content_num++;
			UART_SendByte(content_num);
		...
		if(content_num==8)//接收满
		{
			...
			else if(content[7]==content[6]^content[5]^content[4]^content[3]^content[2]^content[1])//校验		   按位异或(^):如果两个相应的二进制位值不同则为1,否则为0。
			{ 
				content_target=0;//存储内容位
				content_target|=content[3];						  //按位或
...
/*23*/  else if(content[1]<24)				
			...
			content_num=0;//完成一次接收后，将接收序号打乱，等待下一次接收才有效
		}
	}
	if(TI)
	{
...
void motor_move(int motor_move_n)
{
	if(motor_move_n<-150)
	{
		CCAP0H=0;motor_CCW;			 				//PWM0比较值设为0，反转模式（PWM0持续高电位，<-150时超出调速范围，全速反转）
	}
	else if(motor_move_n<0)
	{
		CCAP0H=150+motor_move_n;motor_CCW;			//PWM0比较值设置为150+motor_move_n,反转模式
	}
	else if(motor_move_n==0)
	{
		CCAP0H=0; motor_brake;						//PWM0比较值设置为0,双高电位，刹车
	}
	else if(motor_move_n<150)
	{
		CCAP0H=150-motor_move_n;motor_CW;			//比较值为正值，正转，比较值为150-motor_move_n，即值越大转速越快 
	}
	else
	{
		CCAP0H=0;motor_CW;							//PWM0比较值设为0，反转模式（PWM0持续高电位，>150时超出调速范围，全速正转）																	  
	}
}
void UART_SendByte(unsigned char Byte)
{
